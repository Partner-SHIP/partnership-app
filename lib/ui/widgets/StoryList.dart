...
class StoryList extends StatelessWidget {
  ...
  double _height;
  StoryList({@required List<StoryListItem> items, double height = 450})
      : _list = items,
        _height = height;
...
const TextStyle storyHeaderTextStyle = TextStyle(
    fontSize: 20,
    fontFamily: "Roboto",
    color: Colors.white,
    shadows: [Shadow(color: Colors.black, blurRadius: 10)]);
const TextStyle storyDescriptionTextStyle = TextStyle(
    fontFamily: "Roboto",
    color: Colors.white,
    shadows: [Shadow(color: Colors.black, blurRadius: 10)]);
...
class StoryListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Container result =
        _buildContainer(width: MediaQuery.of(context).size.width);
